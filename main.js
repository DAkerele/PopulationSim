$(document).ready(function() {
    ...
    function draw(e) { //draws nodes on click event
        ...
        if (NodeCount == 11) {
            ...
            var isInCircle = pointInCircle(posX, posY, 25);
            ...
            for (var i = 0; i < Nodes.length; i++) {
                if (Nodes[i].NodeNum != currentSelectedNode) {
                    Nodes[i].isSelected = false;
                }

                if (Nodes[i].NodeNum == isInCircle.NodeNum) {
                    Nodes[i].isSelected = true;
                }
               
                //currentSelectedNode = Nodes[i].NodeNum;
            }
        }
    ...
    function pointInCircle(x, y, radius) { //checks if mouse click is located within a node
        for (var i = 0; i < Nodes.length; i++) {
            var distance = (x - Nodes[i].CoordX) * (x - Nodes[i].CoordX) + (y - Nodes[i].CoordY) * (y - Nodes[i].CoordY);
            if (distance <= radius * radius) {
                currentSelectedNode = Nodes[i];
                return Nodes[i];
            }
        }
    }

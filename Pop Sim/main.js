$(document).ready(function() {
    ...
    var context = canvas.getContext("2d");$(document).ready(function() {
        ...
        if (!running) {
            ...
            for (var i = 0; i < allNodes.length; i++) {
                ...
                if (allNodes[i].NodeNum == isInCircle.NodeNum) { //creates red rings surrounding selected node
                    ...
                    $("#NumRuns").val(currentSelectedNode.numRuns);
                    $("#Starting").val(parseFloat((currentSelectedNode.startPer*100.0)));
                    $("#NumGenerations").val(currentSelectedNode.genNum);
                    $("#StartingPop").val(currentSelectedNode.startPop);
                    $("#PPSurvival").val(parseFloat((currentSelectedNode.plusplusS*100.0)));
                    $("#PMSurvival").val(parseFloat((currentSelectedNode.plusminusS*100.0)));
                    $("#MMSurvival").val(parseFloat((currentSelectedNode.minusminusS*100.0)));
                        debugger;
    ...
    function fieldCheck(node){
            ...
            if(node.startPer != parseFloat($("#Starting").val()/100)){node.isConfirm = false;}
            else if(node.genNum != parseInt($("#NumGenerations").val())){node.isConfirm = false;}
            else if(node.startPop !=  parseInt($("#StartingPop").val())){node.isConfirm = false;}
            else if(node.plusplusS !=  parseFloat($("#PPSurvival").val()/100)){node.isConfirm = false;}
            else if(node.plusminusS !=  parseFloat($("#PMSurvival").val()/100)){node.isConfirm = false;}
            else if(node.minusminusS !=  parseFloat($("#MMSurvival").val()/100)){node.isConfirm = false;} 
            else if(node.numRuns != parseInt($("#NumRuns").val())){node.isConfirm = false;}
